# Для варианта c 7-ю ячейками :
backpack = []
inventory = [['empty' for _ in range(7)]]  # инвентарь
size_backpack = 1*7  # размер рюкзака (1*7)
weight = 0  # общий вес в рюкзаке
points = 26  # начальное количество очков (можно менять!!!)
...
# поскольку в данном варианте он ничем не болеет, то проверку на антидот и тд не делаем
...
# Результаты для варианта c 7-ю ячейками:
if total_points > 0:
    print('Весь инвентарь (1*7):')
    for i in inventory:
        print(i)
    print(f'Очки выживания: {total_points}')
else:
    print('Количество очков выживания отрицательное или равно нулю')

# ИТОГ: если кол-во очков выживания при рюкзаке в 7 ячеек больше 25, то общ. кол-во очков положительное
